...
int main () {
    ...
    // initialize radii with a constant value (can be modified later)
    std::vector<double> radii(numParticles, 1.0); // all particles have radius 1.0

    // write to file
    std::string file ="initial/initial1000.vtk";
    writeVTKFile(file, positions, velocities, masses, radii);
