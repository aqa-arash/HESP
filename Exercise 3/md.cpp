...
int main(int argc, char** argv) {
    ...
    std::cout << "Checking OpenACC device configuration..." << std::endl;

    #ifdef _OPENACC
        acc_device_t dev_type = acc_get_device_type();
        switch (dev_type) {
            case acc_device_nvidia:
                std::cout << "Running on GPU (NVIDIA)" << std::endl;
                break;
            case acc_device_host:
                std::cout << "Running on CPU (host fallback)" << std::endl;
                break;
            default:
                std::cout << "Running on unknown OpenACC device type" << std::endl;
                break;
        }
    #else
        std::cout << "OpenACC not enabled — running on CPU without parallel acceleration." << std::endl;
    #endif


    // Launch a parallel region with visible output
    std::cout << "Launching parallel loop..." << std::endl;

    #pragma acc parallel loop
    for (int i = 0; i < 8; ++i) {
        printf("Thread executing iteration %d\n", i);
    }
    ...
    #pragma acc data \
        copyin(mass[0:numParticles]) \
        copy(pos_new[0:total_size], pos_old[0:total_size], \
            vel_new[0:total_size], vel_old[0:total_size], \
            acc[0:total_size], force[0:total_size], \
            cell_data[0:cell_data_size], particle_cell[0:N])
    {
        ...
        for (int timestep = 0; timestep < timeStepCount; ++timestep) {
            ...
            if (printInterval > 0 && timestep % printInterval == 0) {
                ...
                // (GPU data is automatically copied back to CPU for this operation)
                    #pragma acc update self(pos_old[0:total_size], vel_old[0:total_size])
